...
def process_full_coords(instructions):
    x = y = 0
    coords = []
    for c in instructions.split(","):
        ...
        if orientation == 'R':
            coords += ([(x+n, y) for n in range(1, quantity+1, 1)])
            x += quantity
        elif orientation == 'U':
            coords += ([(x, y+n) for n in range(1, quantity+1, 1)])
            y += quantity
        elif orientation == 'L':
            coords += ([(x-n, y) for n in range(1, quantity+1, 1)])
            x -= quantity
